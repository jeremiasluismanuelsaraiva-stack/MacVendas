...
(function () {
    ...
    async function salvarConfiguracoes() {
        ...
        if (dados.terminal.ativo && !dados.terminal.api) {
            alert("Defina a API do terminal antes de ativá-lo.");
            return;
        }

        if (
            dados.terminal.api &&
            !/^https?:\/\//i.test(dados.terminal.api)
        ) {
            alert("A API deve começar com http:// ou https://");
            return;
        }
        ...
        try {
            ...
            let mensagem = erro?.message ?? erro ?? "Erro ao guardar as configurações.";
            if (typeof mensagem === "object") {
                try {
                    mensagem = JSON.stringify(mensagem);
                } catch (_) {
                    mensagem = "Erro retornado pelo servidor.";
                }
            }
